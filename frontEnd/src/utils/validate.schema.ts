...
export const AddDegree1Schema = Yup.object().shape({
    ...
    degree_name: Yup.string()
        .required('Tên bằng không được để trống'),
    degree_type: Yup.string()
        .required('Loại bằng tên không được để trống'),
    graduation_year: Yup.string()
        .required('Năm tốt nghiệp không được để trống'),
    gpa: Yup.string()
        .required('GPA không được để trống'),
});
